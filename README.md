...
| EXTENSIONS_DIR         | No        | /tmp/extensions/resources | Directory where extracted extension files are copied. Must match the volume `mountPath` shared with the Argo CD server container. Set to `/tmp/extensions` when the shared volume is mounted at that path (for example, the Argo CD Helm chart `server.extensions` defaults). |
